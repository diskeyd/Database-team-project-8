...
3차 과제(`docs/03-3차과제-릴레이션스키마.pdf`)에서 작성한 관계 스키마를 **MySQL 8.x / MariaDB 10.x** 로 구현한 결과물. 6-1주차 피드백, 11-1주차 강의 매핑 규칙, **3차 피드백(거래내역 약한 개체화)** 까지 반영한 정정본입니다.
...
| **`거래내역` 약한 개체 처리** | ER·매핑 정정 (이중 사각형, 발생 이중 마름모, 거래ID 부분키). SQL은 계좌 패턴 따라 surrogate PK 유지 + 코멘트로 trade-off 명시 | 3차 피드백: "약한 개체가 강한 개체가 되는 건 논리적으로 드물다" |
...
- **1:N 관계** (소속/분류/기준/관리): 외래키 컬럼 직접 보유
- **약한 개체 식별 관계** (소유→계좌, 발생→거래내역): 부모 NOT NULL 외래키로 종속성 보존, MySQL 편의상 surrogate PK 채택 (매핑 다이어그램은 결합 기본키)
